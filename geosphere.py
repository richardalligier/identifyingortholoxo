...
def my_distance_ortho(lat1,lon1,lat2,lon2,latm,lonm): # assume that m points are closse to p1 and p2
    ...
    if gc.is_zero_length():
        dist_rad = np.array([pygplates.GreatCircleArc(start_point=p1,end_point=pmi).get_arc_length() for pmi in pm])
    else:
        prot = gc.get_rotation_axis_lat_lon()
        dist_rad = np.abs(np.pi*0.5-np.array([pygplates.GreatCircleArc(start_point=prot,end_point=pmi).get_arc_length() for pmi in pm]))
...
def my_distance_loxo(lat1,lon1,lat2,lon2,latm,lonm,nptsfactor=20):
    ...
    ps = np.linspace(p1,p2,npts)
    lon,lat=proj.transform(ps[:,0],ps[:,1],direction=pyproj.enums.TransformDirection.INVERSE)
    segs = [(pygplates.PointOnSphere(lat[i],lon[i]),pygplates.PointOnSphere(lat[i+1],lon[i+1])) for i in range(len(lon)-1)]
    dists =np.array([my_distance_ortho(pstart.to_lat_lon()[0],pstart.to_lat_lon()[1],pend.to_lat_lon()[0],pend.to_lat_lon()[1],latm,lonm) for (pstart,pend) in segs])
    ...
    return dists.min(axis=0)#6370997 * np.array([  pygplates.GeometryOnSphere.distance(gc,pmi) for pmi in pm])
