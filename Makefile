...
FOLDER_DETECTED = $(FOLDER)/detected_alpha_$(MODEL)_slope_$(TIMESPLIT)_$(SMOOTH)_$(TRACK_TOLERANCE_DEGREES_LONGEST)_$(THRESH_IOU)_$(THRESH_SLOPE)_$(THRESH_BORDER)
...
#	python3 compute_stats.py -folderfigures $(FOLDER_FIGURES) -conflict $(FOLDER)/detectedref.parquet -detectedref $(FOLDER_DETECTEDREF) -detectedother $(FOLDER_DETECTED) -r $(R) -dolmax $(DOLMAX)
	python3 detect_classic.py -timesplit $(TIMESPLIT) -smooth $(SMOOTH) -angle_precision $(ANGLE_PRECISION) -min_distance $(MIN_DISTANCE) -thresh_iou $(THRESH_IOU) -flightplans $(FOLDER)/detectedref.parquet -thresh_border $(THRESH_BORDER) -folderfigures $(FOLDER_FIGURES) -model $(MODEL) -r $(R) -dolmax $(DOLMAX) -identifiedcsv classic.csv